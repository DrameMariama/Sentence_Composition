...
from fairseq.models import (FairseqDecoder, FairseqEncoder, FairseqEncoderDecoderModel, FairseqIncrementalDecoder,
                            FairseqEncoderModel, FairseqModel, register_model, register_model_architecture,
                            )
from fairseq.modules import (AdaptiveSoftmax, LayerNorm, PositionalEmbedding, TransformerDecoderLayer,
...
def encode_sentence(sentences):
    tokens = collate_tokens([MODEL.encode(sentence) for sentence in sentences], pad_idx=1)  

    tensor =[MODEL.extract_features(token) for token in tokens]  ## Roberta first layer correspond to the embedding layer
    tensor = torch.stack(tensor)
    tensor = tensor.squeeze(1)  #B x T x C
    tensor = tensor.transpose(0, 1) # T x B x C
    tensor = torch.mean(tensor, dim=0)
    tensor = tensor.unsqueeze(dim=0)
    return tensor #1xBxC

@register_model('new_transformer')
class TransformerModel(FairseqEncoderDecoderModel):
    ...
    def build_encoder(cls, args, src_dict, embed_tokens):
        return New_TransformerEncoder(args, src_dict, embed_tokens)
...
@register_model('new_transformer_align')
class TransformerAlignModel(TransformerModel):
    ...
    def build_model(cls, args, task):
        ...
        new_transformer_align(args)

        transformer_model = TransformerModel.build_model(args, task)
        return TransformerAlignModel(transformer_model.encoder, transformer_model.decoder, args)

    def forward(self, embedded_sentences, ids, src_tokens, src_lengths, prev_output_tokens):
        encoder_out = self.encoder(embedded_sentences, ids, src_tokens, src_lengths)
...
class New_TransformerEncoder(FairseqEncoder):
    """
    ...
    def forward_embedding(self, src_tokens):
        ...
        # list_src_tokens = []
        # list_full_sents = []
        # sep_token = self.dictionary.index('[SEP]')
       
        # for i in range(src_tokens.size(0)):
        #     j = 0
        #     while j < src_tokens.size(1) and src_tokens[i][j].item() != sep_token:
        #         j += 1
        #     tuple_tensors = torch.split_with_sizes(src_tokens[i], [j, src_tokens.size(1)-j], dim=0)
        #     list_src_tokens.append(tuple_tensors[1][1:])
        #     #print(tuple_tensors[1][1:].size())
        #     list_full_sents.append(self.dictionary.string(tuple_tensors[0]))
        # max_row_src_tokens = max([len(row) for row in list_src_tokens])
        
        
        # new_list_src_tokens = []
        # for t1 in list_src_tokens:
        #     t1 = torch.cat((t1, torch.ones(max_row_src_tokens - t1.size(0),dtype=torch.int64, device=t1.device)*self.dictionary.pad()), dim=0)
        #     new_list_src_tokens.append(t1)
        
       
        # src_tokens_cloze_questions = torch.stack(new_list_src_tokens)
        
        # encoded_full_sent = encode_sentence(list_full_sents)
        # embed = self.embed_scale * self.embed_tokens(src_tokens_cloze_questions)
        # if self.embed_positions is not None:
        #     x = embed + self.embed_positions(src_tokens_cloze_questions)
        # x = F.dropout(x, p=self.dropout, training=self.training)
        # return x, embed, encoded_full_sent

    def forward(self, src_tokens, src_lengths, cls_input=None, return_all_hiddens=False, **kwargs):
        """
        Args:
            src_tokens (LongTensor): tokens in the cloze question of shape
                `(batch, src_len)`
            src_lengths (torch.LongTensor): lengths of each source sentence of
                shape `(batch)`
            full_sents_encoded (tensor of complete sentences encoded) shape (batch, C)
            ids  (LongTensor) id of sentences in the batch
        ...
        encoded_samples = []
        ids = kwargs['id']
        embedded_sentences = kwargs['embed_sent']
        row_to_keep = np.arange(x.size(1))
        row_to_skip =[]
        max_ind = torch.max(ids).item()
        if max_ind < len(embedded_sentences):
            for i in range(ids.size(0)):
                encoded_samples.append(torch.from_numpy(embedded_sentences[ids[i]]))
        #     else:
        #         row_to_skip.append(i)
        #         # x1 = x[:, :i, :]
        #         # x2 = x[:, i+1:, :]
        #         #x = torch.cat((x1, x2), dim=0)
        # if len(row_to_skip) != 0:
        #     row_to_keep = np.delete(row_to_keep, row_to_skip)
        #     row_to_keep = torch.from_numpy(row_to_keep)
        #     x = x[:, row_to_keep, :]
            encoded_samples = torch.stack(encoded_samples)

            encoded_samples = encoded_samples.to(x.device)
            encoded_samples = encoded_samples.type(x.type())
            encoded_samples = encoded_samples.unsqueeze(0)
            #encoded_samples = encoded_samples.half()
        

            return {
                'encoder_out': torch.cat((x, encoded_samples), dim=0),  # 1 x B x C
...
@register_model_architecture('new_transformer', 'new_transformer')
...
@register_model_architecture('new_transformer', 'new_transformer_args')
def new_transformer_args(args):
    args.encoder_embed_dim = getattr(args, 'encoder_embed_dim', 1024)
    args.encoder_ffn_embed_dim = getattr(args, 'encoder_ffn_embed_dim', 1024)
...
@register_model_architecture('new_transformer_align', 'new_transformer_align')
def new_transformer_align(args):
